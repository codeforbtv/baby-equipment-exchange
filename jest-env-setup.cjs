...
try {
    ...
    process.env.NEXT_PUBLIC_EMULATOR_FIRESTORE_PORT ??= String(
        emu.firestore?.port ?? 8080
    );
    process.env.NEXT_PUBLIC_EMULATOR_AUTH_PORT ??= String(
        emu.auth?.port ?? 9099
    );
    process.env.NEXT_PUBLIC_EMULATOR_STORAGE_PORT ??= String(
        emu.storage?.port ?? 9199
    );
