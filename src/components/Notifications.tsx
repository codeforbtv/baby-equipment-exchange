...
const formatShortDate = (date: DateLike): string => {
    return (
        toDateValue(date)?.toLocaleDateString('en-US', {
            month: 'short',
            day: 'numeric'
        }) ?? ''
    );
...
const formatGroupDate = (date: DateLike): string =>
    formatShortDate(date) || 'Unknown date';

const getDateTime = (date: DateLike): number =>
    toDateValue(date)?.getTime() ?? Number.MAX_SAFE_INTEGER;

const getDateKey = (date: DateLike): string =>
    toDateValue(date)?.toISOString().slice(0, 10) ?? 'unknown-date';

const compareNames = (a: string, b: string): number =>
    a.localeCompare(b, undefined, { sensitivity: 'base' });

const compareDates = (a: DateLike, b: DateLike): number =>
...
const compareDonationName = (a: Donation, b: Donation): number => {
    ...
    return (
        compareNames(aName, bName) ||
        compareNames(a.tagNumber ?? '', b.tagNumber ?? '') ||
        compareNames(a.id, b.id)
    );
};

const getEarliestDate = (datesToCompare: DateLike[]): Date | null => {
    const dates = datesToCompare
        .map((date) => toDateValue(date))
        .filter((date): date is Date => Boolean(date));
    if (dates.length === 0) return null;
    return new Date(Math.min(...dates.map((date) => date.getTime())));
};

const getSubmissionDate = (submission: Donation[]): Date | null =>
    getEarliestDate(submission.map((donation) => donation.createdAt));

const getSubmissionKey = (submission: Donation[]): string =>
    submission[0]?.bulkCollection ||
    `standalone-${submission[0]?.id ?? 'unknown'}`;

const getOrderRequestedDate = (order: Order): Date | null =>
    getEarliestDate(order.items.map((item) => item.dateRequested)) ??
    toDateValue(order.createdAt);

const sortDonationsByName = (donations: Donation[]): Donation[] =>
    [...donations].sort(compareDonationName);
...
const groupByDonor = (donations: Donation[]): DonorGroup[] => {
    ...
    const donorMap = new Map<
        string,
        { donorName: string; donorEmail: string; submissions: Donation[][] }
    >();
    ...
    return Array.from(donorMap.entries())
        .map(([donorId, data]) => {
            const submissions = [...data.submissions].sort(
                (a, b) =>
                    compareDates(getSubmissionDate(a), getSubmissionDate(b)) ||
                    compareNames(
                        a[0]?.bulkCollection ?? '',
                        b[0]?.bulkCollection ?? ''
                    )
        ...
        .sort(
            (a, b) =>
                compareNames(a.donorName, b.donorName) ||
                compareDates(
                    getSubmissionDate(a.submissions[0]),
                    getSubmissionDate(b.submissions[0])
                )
        );
};
...
const groupByRequestor = (orders: Order[]): RequestorGroup[] => {
    ...
    return Array.from(map.entries())
            ...
            const orderGroups = Array.from(orderDateMap.entries())
                .map(([dateKey, group]) => ({
                    ...
                    orders: [...group.orders].sort(
                        (a, b) =>
                            compareDates(
                                getOrderRequestedDate(a),
                                getOrderRequestedDate(b)
                            ) || compareNames(a.id, b.id)
                    ),
                    totalItems: group.orders.reduce(
        ...
        .sort(
            (a, b) =>
                compareNames(a.requestorName, b.requestorName) ||
                compareDates(a.orderGroups[0]?.date, b.orderGroups[0]?.date)
        );
};

const groupReservedByRequestor = (
    donations: Donation[]
): PickupRequestorGroup[] => {
    const map = new Map<string, { name: string; donations: Donation[] }>();

    for (const donation of donations) {
        const requestorId =
            donation.requestor?.id || `missing-requestor-${donation.id}`;
        const requestorName = donation.requestor?.name || 'Missing requestor';
        if (!map.has(requestorId))
            map.set(requestorId, { name: requestorName, donations: [] });
        map.get(requestorId)!.donations.push(donation);
    }

    return Array.from(map.entries())
        .map(([requestorId, data]) => {
            const pickupDateMap = new Map<
                string,
                { date: Date | null; donations: Donation[] }
            >();
            for (const donation of data.donations) {
                ...
                if (!pickupDateMap.has(dateKey))
                    pickupDateMap.set(dateKey, { date, donations: [] });
            ...
            return {
                ...
                requestorName: data.name,
                pickupGroups,
                totalItems: pickupGroups.reduce(
                    (sum, group) => sum + group.totalItems,
                    0
                )
            };
        ...
        .sort(
            (a, b) =>
                compareNames(a.requestorName, b.requestorName) ||
                compareDates(a.pickupGroups[0]?.date, b.pickupGroups[0]?.date)
        );
};
...
const Notifications = (props: NotificationsProps) => {
    ...
    const [orderIdToDisplay, setOrderIdToDisplay] = useState<string | null>(
        null
    );
    ...
    return (
        <ProtectedAdminRoute>
            ...
            {!donationIdToDisplay && !userIdToDisplay && !orderIdToDisplay && (
                <>
                    {notifications.donations.length === 0 &&
                                                ...
                                                <>
                                                    ...
                                                    >
                                                        ...
                                                        <Button
                                                            ...
                                                            onClick={() =>
                                                                router.push(
                                                                    `/accept/${group.submissions[0][0].bulkCollection}`
                                                                )
                                                            }
                                                        >
                                                            Review
                                                            ...
                                                            >
                                                                ...
                                                                >
                                                                    ...
                                                                    <Button
                                                                        ...
                                                                        onClick={() =>
                                                                            router.push(
                                                                                `/accept/${submission[0].bulkCollection}`
                                                                            )
                                                                        }
                                                                    >
                                                                        Review
                                ...
                                {requestorGroups.length > 0 ? (
                                    requestorGroups.map((group) => (
                                        ...
                                        >
                                            ...
                                            {group.orderGroups.map(
                                                (
                                                    orderGroup,
                                                    orderGroupIndex
                                                ) => (
                                                    <Box
                                                        key={orderGroup.dateKey}
                                                    >
                                                        {orderGroupIndex >
                                                            0 && <Divider />}
                                                        <Box
                                                                    ...
                                                                    >
                                                                        ...
                                                                        >
                                                                            ...
                                                                            {getOrderRequestedDate(
                                                                                ...
                                                                                ` - ${formatShortDate(getOrderRequestedDate(order))}`}
                                                                        </Typography>
                                                                        <Button
                                                                            size="small"
                                                                            variant="contained"
