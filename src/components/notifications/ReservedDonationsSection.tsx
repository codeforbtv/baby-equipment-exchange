import { Dispatch, SetStateAction } from 'react';
import { Paper, Typography } from '@mui/material';
import NotificationCard from '@/components/NotificationCard';
import styles from '@/components/NotificationCard.module.css';
import { Donation } from '@/models/donation';

interface Props {
    donations: { key: string; donations: Donation[] }[];
    setIdToDisplay: Dispatch<SetStateAction<string | null>>;
}

const ReservedDonationsSection = ({ donations, setIdToDisplay }: Props) => {
    if (donations.length === 0) return null;

    return (
        <div className={styles['notification-section']}>
            <Typography sx={{ marginTop: '1rem', marginBottom: '0.5rem' }} variant="h6">
                Donations waiting for pickup
            </Typography>
            {donations.map(({ key, donations }) => (
                <Paper key={key} className={styles['notification-card--container']} elevation={0}>
                    <Typography variant="h6" sx={{ marginBottom: '1rem' }}>
                        {`${donations[0].donorName} has reserved the following items for pickup:`}
                    </Typography>
                    {donations.map((donation) => (
                        <NotificationCard key={donation.id} donation={donation} type="reserved" setIdToDisplay={setIdToDisplay} />
                    ))}
                </Paper>
            ))}
        </div>
    );
};

export default ReservedDonationsSection;
