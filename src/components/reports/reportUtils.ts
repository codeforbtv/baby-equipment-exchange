 ...
 */
function resolveCellValue(donation: Donation, key: string): string {
    switch (key) {
        case 'tagNumber':
...
export function buildReport(options: BuildReportOptions): ReportData {
    const { donations, selectedColumns, groupByKey, orgLookup = {} } = options;
    ...
    if (!groupByKey) {
        ...
        for (const donation of sorted) {
            data.push(buildRow(donation, selectedColumns, orgLookup));
        }
    } else {
...
function buildRow(
    ...
    return selectedColumns.map((col) => {
        ...
        return { value: resolveCellValue(donation, col.key) };
    });
}
