...
const OrganizationReport = () => {
    ...
    const handleGenerate = async (config: ReportConfig) => {
        ...
        try {
            ...
            if (selectedColumnKeys.includes('storageName')) {
                try {
                    const storageLocations = await getActiveStorage();
                    storageMap = Object.fromEntries(storageLocations.map((s) => [s.id, s.name]));
                } catch {}
            }

            const selectedColumns = selectedColumnKeys.map((key) => availableColumns.find((c) => c.key === key)).filter(Boolean) as ReportColumn[];
            setReportData(buildReport({ donations: filtered, selectedColumns, storageMap, groupByKey: 'orgName', orgLookup }));
        } catch (error) {
