...
//Libs
import {
    adminRequestInventoryItems,
    adminAreDonationsAvailable
...
const AdminCart = () => {
    ...
    const [isUnavailableDialogOpen, setIsUnavailableDialogOpen] =
        useState<boolean>(false);
    const [unavailableDialogContent, setUnavailableDialogContent] =
        useState<string>('');
    ...
    const handleAdminRequestItems = async (
        ...
        try {
            ...
            const unavailableItemIds =
                await adminAreDonationsAvailable(requestedItemIds);

            if (unavailableItemIds.length > 0) {
                const unavailableItems: InventoryItem[] =
                    requestedInventory.filter((item) =>
            ...
            const requestor = activeUsers?.find(
                (user) => user.email === requestorEmailMatch[0]
            );
            if (!requestor)
            ...
            const order = await adminRequestInventoryItems(
                requestedItemIds,
                requestorInfo
            );
                        ...
                        <div className="content--container">
                            ...
                            requestedInventory.length == 0 ? (
                                ...
                                <Box>
                                    ...
                                    <Box
                                        className={
                                            styles['inventoryItem--container']
                                        }
                                    >
                                        {requestedInventory.map(
                                            (inventoryItem, i) => {
