...
function event(invitees: Invitee[]): ScheduledEventWithInvitees {
    return {
        ...
        invitees_counter: {
            ...
            active: invitees.length,
            limit: invitees.length
        },
        created_at: '2026-05-16T12:00:00Z',
        updated_at: '2026-05-16T12:00:00Z',
        event_memberships: [],
...
describe('scheduling matching', () => {
    it('normalizes email aliases conservatively', () => {
        expect(normalizeEmail('Avery.Bennett+bee@gmail.com')).toBe(
            'averybennett@gmail.com'
        );
        expect(normalizeEmail('avery.bennett+bee@example.test')).toBe(
            'avery.bennett@example.test'
        );
    });

    it('scores exact and reversed names without accepting lone first names', () => {
        expect(
            nameSimilarity('Avery Bennett', 'Bennett, Avery')
        ).toBeGreaterThanOrEqual(0.86);
        expect(nameSimilarity('Avery Carter', 'Avery Bennett')).toBeLessThan(
            0.72
        );
    });
